...
// tutti i descrittori rilevanti del sottoalbero. Se flags contiene i bit PWT e/o PWT,
...
extern "C" void main(paddr mbi)
{
	...
	flog(LOG_INFO, "Nucleo di Calcolatori Elettronici, v6.6");
	init_gdt();
...
natl mutex_allocati = 0;
natl alloca_mutex()
{
	natl i;

	if (mutex_allocati >= MAX_MUTEX)
		return 0xFFFFFFFF;
...
extern "C" void c_mutex_wait(natl mux){
	/* tenta di occupare il mutex di identificatore mux. 
	   Se il mutex è già occupato sospende il processo in attesa che il mutex venga prima liberato. 
	   Abortisce il processo in caso di errore. */
	if(!mutex_valido(mux)){
		flog(LOG_WARN, "Mutex non valido \n");
		c_abort_p();
		return;
	}

	des_mutex* mutex = &array_desm[mux];

	// Errore, lo stesso processo tenta di occupare nuovamente il mutex
	if(mutex->owner == esecuzione->id){
		flog(LOG_WARN, "Errore \n");
		c_abort_p();
		return;
	}

	// mutex libero, posso occuparlo	
	if(mutex->owner == 0){
		mutex->owner = esecuzione->id;
	}
	else{ // mutex occupato, devo mettere il processo in attesa
		inserimento_lista(mutex->waiting, esecuzione);
		schedulatore();
	}
}
extern "C" void c_mutex_signal(natl mux){
	/* libera il mutex di identificatore mux. Se qualche altro processo
	   era in attesa, lo risveglia e gli cede il mutex (che in questo caso resta occupato). Gestisce una
	   eventuale preemption. Abortisce il processo in caso di errore.
	*/
	if(!mutex_valido(mux)){
		flog(LOG_WARN, "Mutex non valido \n");
		c_abort_p();
	...
	// errore, il mutex viene liberato da un processo diverso da quello che lo aveva occupato
	if(mutex->owner != esecuzione->id){
		flog(LOG_WARN, "Errore \n");
		c_abort_p();
		return;
	...
	// libero direttamente il mutex
	if(mutex->owner == esecuzione->id && !(mutex->waiting)){
		mutex->owner = 0;
	}

	// libero il mutex, gestendo la preemption
	if(mutex->owner == esecuzione->id && mutex->waiting){
		des_proc *tmp = rimozione_lista(mutex->waiting);
		mutex->owner = tmp->id;
		inspronti();
