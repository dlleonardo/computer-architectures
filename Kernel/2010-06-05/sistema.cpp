...
extern "C" void mutex_wait(natl mux)
{
	if(!mutex_valido(mux)){
	...
	des_mutex* mutex;
	mutex = &array_desm[mux];

	// Mutex gia' occupato da esecuzione
	if(mutex->owner == esecuzione->id){
		flog(LOG_WARN, "Mutex gia' occupato da p \n");
		c_abort_p();
		return;
	}
	...
	// Mutex occupato, mettilo in attesa
	else{
		inserimento_lista(mutex->waiting, esecuzione);
		schedulatore();
	}
}
extern "C" void mutex_signal(natl mux)
{
	if(!mutex_valido(mux)){
		flog(LOG_WARN, "Mutex non valido \n");
		c_abort_p();
	...
	// Il processo e' diverso dall'owner del mutex
	if(esecuzione->id != mutex->owner){
		flog(LOG_WARN, "Mutex owner errato \n");
		c_abort_p();
		return;
	}

	if(mutex->waiting != 0){
		// Devo controllare se ci sono altri proc in attesa
		des_mutex* tmp = rimozione_lista(mutex->waiting);
		mutex->owner = tmp->id;
		inspronti();
