...
extern "C" void c_ceread(natl id, char *buf, natl& quanti, char stop){
	...
	// Bisogna fare il controllo anche su natl& quanti
	// perche' i riferimenti sono indirizzi, quindi
	// potrebbero puntare a spazi privati
	...
	c->quanti = quanti;
	c->buf = buf;
	// La periferica non opera in DMA, ma tramite il registro
	// di stato e registro RBR, quindi non si trasforma
	outputb(1, c->iCTL);
	sem_wait(c->sync);

	quanti = quanti - c->quanti;
	sem_signal(c->mutex);
}
extern "C" void estern_ce(int id){
	...
	for(;;){
		...
		// scrivo il valore appena letto dentro buffer,
		// ci pensa la MMU a tradurre da virtuale a fisico
		// quindi in questa prova non e' necessario
		*c->buf++ = b;
		c->quanti--;
