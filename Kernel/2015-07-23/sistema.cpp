...
/* Permette ad un processo di eliminare la shmem id dal proprio spazio di indirizzamento. Abortisce il processo se la shmem id non è tra quelle
a cui il processo è collegato. */
class Funtore {
	public:
		void operator() (vaddr v, int a){
...
extern "C" void c_shmem_detach(natl id){
	...
	// Dobbiamo scorrere tutta la lista dei des_attached
	// utilizzo un doppio puntatore per fare prima
	des_attached **patt = nullptr;
	// scorriamo, & perche' e' un doppio puntatore, *patt!=null; 
	...
	if(*patt == nullptr){
		flog(LOG_WARN, "Il processo non e' collegato a questa shmem");
		c_abort_p();
		return;
	}
	...
	vaddr begin = (*patt)->start;
	vaddr end = begin + DIM_PAGINA*p_shmem->npag;

	// Devo estrarre dalla lista il des_attached, e farla avanzare
	des_attached *old = *patt;
	*patt = (*patt)->next;
	delete old; // elimino il vecchio elemento

	// Dobbiamo invalidare il TLB, perche' questa volta, essendo stata
	// mappata in precedenza, non sappiamo se qualcuno ci ha acceduto o meno
	// di conseguenza invalidiamo il TLB
	unmap(esecuzione->cr3, begin, end, [](vaddr v, paddr, int) {
				invalida_entrata_TLB(v);
			});
}
//   SOLUZIONE 2015-07-23 )
