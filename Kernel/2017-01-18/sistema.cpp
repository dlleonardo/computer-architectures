...
// ( SOLUZIONE 2017-01-18
	natl b_id; // id dell'ultimo messaggio letto
//   SOLUZIONE 2017-01-18 )
...
// ( SOLUZIONE 2017-01-18
	natl broadcaster;
	natl nmess;		// totale dei messaggi finora, cioe' ultima posizione occupata
	natl arr_msg[MAX_BROADCAST];
	des_proc *lista_listener;
//   SOLUZIONE 2017-01-18 )
// ( ESAME 2017-01-18
};

broadcast_t global_broadcast;
...
//   ESAME 2017-01-18 )
// ( SOLUZIONE 2017-01-18
	global_broadcast.broadcaster = 0;
	global_broadcast.lista_listener = 0;
	global_broadcast.nmess = 0;
//   SOLUZIONE 2017-01-18 )
// ( ESAME 2017-01-18
}
//   ESAME 2017-01-18 )
...
// ( SOLUZIONE 2017-01-18
/* registra il processo per il ruolo dato da role; è un errore se role non specifica né un broadcaster, né un listener, se il processo era già
registrato (per lo stesso o un altro ruolo), o se cè già un broadcaster e si tenta di registrarne un altro; */
extern "C" void c_reg(broadcast_role role){
	if(role != B_BROADCASTER || role != B_LISTENER){
		flog(LOG_WARN, "Ruolo non riconosciuto");
		c_abort_p();
		return;
	}

	// Processo gia' registrato
	if(esecuzione->b_reg != B_NONE){
		flog(LOG_WARN, "Processo gia' registrato");
		c_abort_p();
		return;
	}

	// Errore se il broadcaster e' gia' registrato
	if(global_broadcast.broadcaster_registered && role == B_BROADCASTER){
		flog(LOG_WARN, "Processo gia' registrato");
		c_abort_p();
		return;
	}
	...
	if(!global_broadcast.broadcaster_registered && role == B_BROADCASTER){
		global_broadcast.broadcaster_registered = true;
		global_broadcast.broadcaster = esecuzione->id;
	}
}
/* restituisce il prossimo messaggio di broadcast non ancora letto dal processo; 
se il processo li ha già letti tutti, si blocca in attesa del prossimo; è un errore se il processo non è registrato come listener; */
extern "C" natl c_listen(){

	if(esecuzione->b_reg != B_LISTENER){
		flog(LOG_WARN, "Il processo non e' listener");
		c_abort_p();
		return;
	}

	if(esecuzione->b_id < global_broadcast.nmess){ // c'e' qualcosa che possiamo leggere
		natl new_mess = global_broadcast.arr_msg[esecuzione->b_id];
		esecuzione->contesto[I_RAX] = new_mess;
		esecuzione->b_id++;
	}
	else{ // sospensione del processo
		inserimento_lista(global_broadcast.lista_listener, esecuzione);
		schedulatore();
	}
}
/* invia in broadcast il messaggio msg; è un errore se il processo non è registrato come broadcaster o se il limite di messaggi di broadcast è stato
superato. */
extern "C" void c_broadcast(natl msg){

	if(esecuzione->b_reg != B_BROADCASTER){
		flog(LOG_WARN, "Il processo non e' broadcaster");
		c_abort_p();
		return;
	}

	if(global_broadcast.broadcaster != esecuzione->id){
		flog(LOG_WARN, "Il processo non e' broadcaster");
		c_abort_p();
		return;
	}

	if(sizeof(global_broadcast.arr_msg) >= MAX_BROADCAST){
		flog(LOG_WARN, "Il limite dei messaggi di broadcast e' stato superato");
		c_abort_p();
		return;
	}

	// Invia il messaggio msg a tutti i listener, compresi quelli in attesa
	if(global_broadcast.broadcaster == esecuzione->id){
		// inserisco il messaggio nell'array del global broadcast
		natl aux = global_broadcast.nmess;
		global_broadcast.arr_msg[aux] = msg;
		global_broadcast.nmess++;

		// mando il messaggio ai listeners in coda di attesa
		inspronti();
		while(global_broadcast.lista_listener){
			des_proc *tmp = rimozione_lista(global_broadcast.lista_listener);
			tmp->contesto[I_RAX] = msg;
			tmp->b_id++;
...
// }
#include <type_traits>
template<typename T, typename = std::enable_if_t<!std::is_same_v<T, void(vaddr)>>>
...
extern "C" void main(paddr mbi)
{
	...
	flog(LOG_INFO, "Nucleo di Calcolatori Elettronici, v6.6.2");
	init_gdt();
...
// ( ESAME 2017-01-18
	broadcast_init();
	flog(LOG_INFO, "broadcast inizializzato");
//   ESAME 2017-01-18 )
