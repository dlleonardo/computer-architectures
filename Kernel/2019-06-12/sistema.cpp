...
/*blocca il processo chiamante in attesa che un altro processo provi ad eseguire l’istruzione all’indirizzo rip; restituisce l’id del processo intercet-
tato, o 0xFFFFFFFF se un altro processo sta già aspettando un breakpoint (a qualunque indirizzo); abortisce il processo se rip non appartiene 
all’intervallo [ini utn c, fin utn c) (zona utente/condivisa).*/
struct des_b{
	des_proc *waiting;
	vaddr rip;
	natb original;
...
extern "C" void c_breakpoint(vaddr rip){
	...
	if(deb_b.waiting){
		esecuzione->contesto[I_RAX] = (natq)-1;
		return;
	}

	if(rip < ini_utn_c || rip >= fin_utn_c){
		flog(LOG_WARN, "Non appartiene all'intervallo");
		c_abort_p();
		return;
	}

	// Dobbiamo scrivere 0xCC all'indirizzo che ci e' stato passato
	natb *puntatore = reinterpret_cast<natb*>(c_trasforma(rip));
	deb_b.original = *puntatore;

	// Ogni volta che il processore becca questo byte 0xCC, manda
	// in esecuzione l'eccezione/routine c_breakpoint_exception,
	// quello che vogliamo fare e' in fare in modo che il tutto
	// avviene in modo trasparente, nel senso che dopo la fine
	// dell'eccezione tutto deve tornare come in origine
	*puntatore = 0xCC;
	deb_b.rip = rip;

	deb_b.waiting = esecuzione;
	schedulatore();
}
extern "C" void c_breakpoint_exception(int tipo, natq errore, vaddr* rip){
	// rip punta gia' all'istruzione successiva, quindi devo sommare +1
	if(deb_b.waiting == 0 || deb_b.rip != *rip-1){
		gestore_eccezioni(tipo, errore, *rip);
		return;
	}
	// tutto deve tornare come prima in origine, si usa la trasforma
	// perche' accedendo all'indirizzo virtuale possiamo scrivere in
	// deb_b.rip
	natb *ptr = reinterpret_cast<natb*>(c_trasforma(deb_b.rip));
	*ptr = deb_b.original;

	(*rip)--;
	// stiamo scrivendo nel contesto di chi ha inviato la primitiva breakpoint
	deb_b.waiting->contesto[I_RAX] = esecuzione->id;

	inspronti();	// preemption
	inserimento_lista(pronti, deb_b.waiting);
	deb_b.waiting = 0;
	schedulatore();
}
//   SOLUZIONE 2019-06-12 )
