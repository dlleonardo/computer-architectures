...
/* Permette ad un processo di aggiungere la shmem id al proprio spazio di indirizzamento e ne restituisce l’indirizzo di partenza. 
Abortisce il processo se id non corrisponde ad una shmem esistente. Restituisce 0 se non è stato possibile
aggiungere la zona (perché non vi sono indirizzi liberi sufficienti a contenerla). */
class Funtore {
	private:
		natl fn; // id del primo frame number, la prima volta
	public:
		// costruttore, dobbiamo scorrere una lista,
		// questo Funtore deve sapere qual'e' il primo 
		// elemento della lista, di conseguenza dobbiamo
		// passarglielo, sarebbe fn
		Funtore(natl n) : fn(n){} 
		paddr operator() (vaddr v){ // getpaddr
			natl cfn = fn;
...
extern "C" void c_shmem_attach(natl id){
	...
	if(!shmem_valid(id)){
		flog(LOG_WARN, "Shmem non esistente");
		c_abort_p();
		return;
	}

	des_shmem *p_shmem = &array_desshmem[id];

	vaddr begin = esecuzione->avail_addr;
	vaddr end = esecuzione->avail_addr + (DIM_PAGINA*(p_shmem->npag));

	// Controllo sul wrap around
	if(end < begin || end >= fin_utn_c){
		flog(LOG_WARN, "Indirizzi non disponibili");
		c_abort_p();
		return;
	}

	vaddr res_map = map(esecuzione->cr3, begin, end, BIT_US | BIT_RW, Funtore(p_shmem->first_frame_number));
	/* chiamata map con lamba: 
	natl fn = p_shmem->first_frame_number;
	...
	if(res_map != end){
		unmap(esecuzione->cr3, begin, res_map, [](vaddr, paddr, int){}); // getpaddr sintassi con lambda
